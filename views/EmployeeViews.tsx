...
import { Calendar, Utensils, FileText, ChevronRight, ChevronLeft, AlertCircle, MessageCircle, X, Clock, CheckCircle, XCircle, Send, MessageSquare, Info } from 'lucide-react';
...
export const OrderLunchView: React.FC = () => {
  ...
  const handleOrder = () => {
    ...
    // (Validation logic omitted for brevity, but functionality remains)
  ...
  return (
    <div className="space-y-6 pb-32 relative">
      <div className="bg-yellow-100 border-2 border-yellow-400 p-4 text-center rounded-lg shadow-sm"><h1 className="text-xl md:text-2xl font-extrabold text-gray-900 uppercase tracking-tight mb-2">Facey Commodity Limited - Lunch Menu</h1><p className="font-bold text-yellow-800 whitespace-pre-wrap">{menuForDay.notes || "Menu details for this day have not been published yet."}</p></div>

      {/* DATE SELECTOR (Darker Card) */}
      <div className="bg-gray-50 p-6 rounded-xl shadow-sm border border-gray-200 mt-8">
        <div className="flex justify-between items-center mb-4">
            <div className="flex flex-col"><h2 className="text-xl font-bold text-gray-800 flex items-center"><Calendar className="w-5 h-5 mr-2 text-blue-600" />Menu For the Week</h2><span className="text-xl md:text-2xl font-bold text-blue-600 ml-7 mt-1">{getDynamicRangeLabel()}</span></div>
            <div className="flex items-center space-x-2 bg-gray-200 rounded-full p-1"><button onClick={() => setWeekOffset(weekOffset - 1)} className="p-1 hover:bg-white rounded-full shadow-sm"><ChevronLeft className="w-4 h-4" /></button><button onClick={() => setWeekOffset(weekOffset + 1)} className="p-1 hover:bg-white rounded-full shadow-sm"><ChevronRight className="w-4 h-4" /></button></div>
        </div>
        <div className="flex gap-4 overflow-x-auto pb-6 pt-2">
            {weekDates.map((dateObj) => { const dateStr = toLocalISOString(dateObj); const isSelected = dateStr === selectedDate; const dayName = dateObj.toLocaleDateString('en-US', { weekday: 'long' }); const dayNum = dateObj.getDate(); const hasMenu = menus.some(m => m.date === dateStr); return (<button key={dateStr} onClick={() => setSelectedDate(dateStr)} className={`flex flex-col items-center justify-center flex-shrink-0 w-36 h-28 p-2 rounded-xl border-2 transition-all relative gap-1 ${isSelected ? 'border-blue-600 bg-blue-600 text-white font-extrabold transform scale-105 shadow-xl z-10' : 'border-gray-200 bg-white hover:bg-gray-100 text-gray-600'}`}><span className="text-xs font-bold uppercase tracking-wide">{dayName}</span><span className="text-3xl font-bold">{dayNum}</span>{hasMenu && <div className={`w-2 h-2 rounded-full ${isSelected ? 'bg-white' : 'bg-green-500'}`}></div>}</button>); })}
        </div>
      </div>
      ...
      {/* --- NEW SPLIT LAYOUT (Grid + Info Sidebar) --- */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          
          {/* LEFT: The Menu Grid (Take up 3/4 space) */}
          <div className="lg:col-span-3">
              <MenuGrid items={menuForDay.items} selectedItemIds={selectedItems} onItemClick={toggleItem} />
          </div>

          {/* RIGHT: The "Wasted Space Filler" (Summary & Notes) */}
          <div className="lg:col-span-1 space-y-6">
              {/* Nutritional Summary Card */}
              <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                  <h4 className="font-bold text-gray-700 flex items-center mb-4"><Info className="w-4 h-4 mr-2" /> Meal Summary</h4>
                  {selectedItems.length === 0 ? (
                      <p className="text-sm text-gray-400 italic">Select items to see nutritional info.</p>
                  ) : (
                      <div className="space-y-2">
                          <div className="flex justify-between text-sm"><span className="text-gray-600">Items Selected:</span><span className="font-bold">{selectedItems.length}</span></div>
                          <div className="flex justify-between text-sm"><span className="text-gray-600">Total Calories:</span><span className="font-bold text-blue-600">{totalCalories} cal</span></div>
                          <div className="mt-4 pt-4 border-t border-gray-100">
                              <p className="text-xs text-gray-400">Dietary Badges:</p>
                              <div className="flex gap-2 mt-1">
                                  {/* Just examples of what they picked */}
                                  {selectedItemsData.flatMap(i => i.dietaryInfo || []).filter((v, i, a) => a.indexOf(v) === i).map(tag => (
                                      <span key={tag} className="px-2 py-1 bg-green-50 text-green-700 text-[10px] rounded-full border border-green-100">{tag}</span>
                                  ))}
                              </div>
                          </div>
                      </div>
              ...
              <div className="bg-white p-5 rounded-xl shadow-sm border-l-4 border-blue-600">
                  ...
                  <textarea 
                      ...
                      className="w-full pl-3 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-blue-500 h-32 bg-gray-50 text-gray-800 text-sm resize-none"
      ...
      <div className={`fixed inset-y-0 right-0 w-96 bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-l border-gray-200 ${isIssueDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}><div className="h-full flex flex-col"><div className="p-6 bg-slate-900 border-b border-slate-800 flex justify-between items-center"><h2 className="text-xl font-bold text-white flex items-center"><AlertCircle className="w-6 h-6 mr-2 text-red-400" />Report Issue</h2><button onClick={() => setIsIssueDrawerOpen(false)} className="p-2 hover:bg-slate-800 rounded-full text-slate-400 transition-colors"><X className="w-6 h-6" /></button></div><div className="flex-1 overflow-y-auto p-6 space-y-6"><p className="text-gray-600 text-sm">Did you receive the wrong order? Please let the kitchen know.</p>{myIssues.length > 0 && (<div className="space-y-4"><h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider">Your Reports Today</h3>{myIssues.map(issue => (<div key={issue.id} className="bg-gray-50 p-4 rounded-lg border border-gray-200 text-sm"><p className="font-bold text-gray-800 mb-1">"{issue.issue}"</p><div className="flex justify-between items-center mt-2"><span className="text-xs text-gray-400">{new Date(issue.timestamp).toLocaleTimeString()}</span>{issue.chefResponse ? <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full font-bold">Resolved</span> : <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full">Pending</span>}</div>{issue.chefResponse && <div className="mt-3 p-3 bg-green-50 text-green-800 rounded border border-green-100 flex items-start"><MessageCircle className="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" /><span><strong>Chef:</strong> {issue.chefResponse}</span></div>}</div>))}</div>)}<div><label className="block text-sm font-bold text-gray-700 mb-2">Describe the issue</label><textarea value={issueText} onChange={e => setIssueText(e.target.value)} className="w-full h-32 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-500 outline-none resize-none text-gray-900 bg-white" placeholder="Type your message here..." /></div></div><div className="p-6 border-t border-gray-100 bg-gray-50"><button onClick={handleReportIssue} disabled={!issueText.trim()} className={`w-full py-3 px-4 rounded-lg font-bold text-white shadow-lg transition-all ${issueText.trim() ? 'bg-slate-900 hover:bg-black transform hover:scale-[1.02]' : 'bg-gray-400 cursor-not-allowed'}`}>Submit Report</button></div></div></div>{isIssueDrawerOpen && <div className="fixed inset-0 bg-black bg-opacity-25 z-40" onClick={() => setIsIssueDrawerOpen(false)}></div>}</div>
  );
};

// ... (OrderHistoryView, MessagesView, FeedbackView - No changes needed, they will inherit the darker background from Layout)
export const OrderHistoryView: React.FC = () => {
  ...
  return (<div className="space-y-6"><div className="bg-white p-6 rounded-xl shadow-sm border-b border-gray-200"><h2 className="text-2xl font-bold text-gray-800">My Order History</h2></div>{myOrders.length === 0 ? <div className="text-center py-20 bg-white rounded-xl shadow-sm"><Clock className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">You haven't placed any orders yet.</p></div> : <div className="bg-white rounded-xl shadow-sm overflow-hidden"><table className="w-full text-left"><thead className="bg-gray-50 border-b border-gray-100"><tr><th className="p-4 text-gray-600">Date</th><th className="p-4 text-gray-600">Items</th><th className="p-4 text-gray-600">Status</th></tr></thead><tbody className="divide-y divide-gray-100">{myOrders.map(order => (<tr key={order.id} className="hover:bg-gray-50"><td className="p-4"><div className="font-medium text-gray-800">{new Date(order.date).toLocaleDateString()}</div></td><td className="p-4"><p className="text-sm text-gray-600">{getItemNames(order.menuId, order.selectedItemIds)}</p></td><td className="p-4"><span className={`px-3 py-1 rounded-full text-xs font-medium ${order.status === 'Confirmed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{order.status}</span></td></tr>))}</tbody></table></div>}</div>);
};
