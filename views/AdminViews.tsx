              ...
              {activeIssues.map(issue => (
                  <div key={issue.id} className="border border-red-500/30 bg-red-900/10 rounded-xl p-6 shadow-sm">
                      <div className="flex justify-between items-start mb-4">
                          <div>
                              <span className="bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wide mr-2">New Alert</span>
                              <span className="text-xs text-slate-400 font-mono">{new Date(issue.timestamp).toLocaleTimeString()}</span>
                          </div>
...
export const AdminMenuManager: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-20 relative">
      ...
      {/* NOTES - Made Brighter for Attention */}
      ...
      <div className="space-y-4">
          <div className="flex items-center justify-between">
              {/* FIXED: Bright Blue Text for Visibility */}
              <h3 className="text-xl font-bold text-blue-400">Preview: {formatDateDisplay(selectedDate)}</h3>
              {/* FIXED: High Contrast Red Badge */}
              <span className="text-xs font-bold bg-red-900/50 text-red-200 px-3 py-1.5 rounded border border-red-800 uppercase tracking-wide">
                  Click items below to remove them
              </span>
          </div>
