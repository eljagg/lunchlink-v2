...
import { Plus, Trash2, MessageCircle, Utensils, Search, Copy, CheckCircle, Clock, AlertCircle } from 'lucide-react';
import { MenuGrid } from '../components/MenuGrid';

// --- DATE HELPER: Forces YYYY-MM-DD format ---
...
// --- DISPLAY HELPER: Fixes the "Monday shows as Sunday" bug ---
const formatDateDisplay = (dateStr: string) => {
    // Append T12:00:00 to force Noon, preventing timezone rollback
...
export const AdminMenuManager: React.FC = () => {
  const { menus, masterFoodItems, addMenu, updateMenu, copyMenuFromDate } = useStore();
  ...
  const handleCopyPrevious = () => {
      ...
      const prevDate = toLocalISOString(dateObj);
      if (confirm(`Overwrite ${selectedDate} with menu from ${prevDate}?`)) copyMenuFromDate(prevDate, selectedDate);
  };

  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-center"><h1 className="text-3xl font-extrabold text-gray-800">Menu Planner</h1><button onClick={handleCopyPrevious} className="flex items-center bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-2 rounded-lg shadow-sm font-medium transition-colors"><Copy className="w-4 h-4 mr-2" /> Copy Yesterday's Menu</button></div>
              ...
              {weekDates.map((dateObj) => { 
                  ...
                  return (
                      <button 
                          ...
                          className={`flex flex-col items-center justify-center flex-shrink-0 w-24 h-24 p-2 rounded-xl border-2 transition-all relative gap-1 ${isSelected ? 'border-blue-600 bg-blue-600 text-white font-bold shadow-lg transform scale-105' : 'border-gray-100 bg-gray-50 hover:bg-gray-100 text-gray-500'}`}
                      >
      ...
      <div className="space-y-4">
          <div className="flex items-center justify-between">
              {/* FIXED: Uses formatDateDisplay so Tuesday shows as Tuesday */}
              <h3 className="text-xl font-bold text-gray-800">Preview: {formatDateDisplay(selectedDate)}</h3>
              <span className="text-xs text-gray-400 bg-white px-2 py-1 rounded border">Click items below to remove them</span>
          </div>
          <div className="relative group ring-4 ring-gray-100 rounded-xl p-1"><MenuGrid items={menuItems} selectedItemIds={[]} onItemClick={(id) => { if(confirm("Remove this item from the menu?")) handleRemoveItem(id); }} /></div>
      </div>
    </div>
  );
};
...
export const KitchenMasterDatabase: React.FC = () => {
  const { masterFoodItems, addMasterItem, deleteMasterItem } = useStore();
  const [name, setName] = useState('');
  const [category, setCategory] = useState<MenuCategory>('Protein');
  const [search, setSearch] = useState('');

  const handleAdd = () => { if(!name.trim()) return; addMasterItem({ id: 'mfi_' + Date.now(), name, category, description: 'Chef Added', calories: 0, dietaryInfo: [], isAvailable: true }); setName(''); alert("Item added to Master Database."); };
  ...
  return (
    <div className="space-y-8 pb-20">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200"><h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center"><Utensils className="w-6 h-6 mr-2 text-blue-600" /> Add New Master Item</h2><div className="flex flex-col md:flex-row gap-4 items-end"><div className="flex-1 w-full"><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Item Name</label><input type="text" placeholder="e.g. Curried Shrimp" className="w-full p-3 border rounded-lg bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500" value={name} onChange={e => setName(e.target.value)} /></div><div className="w-full md:w-64"><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Category</label><select className="w-full p-3 border rounded-lg bg-gray-50" value={category} onChange={e => setCategory(e.target.value as MenuCategory)}><option value="Protein">Protein</option><option value="Carbohydrate">Carbohydrate</option><option value="Fibre">Fibre / Vegetable</option><option value="Sandwiches">Sandwiches</option></select></div><button onClick={handleAdd} disabled={!name} className={`px-8 py-3 rounded-lg font-bold text-white shadow-md transition-all ${name ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-300'}`}>Add Item</button></div></div>
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200"><div className="p-4 border-b border-gray-100 flex items-center bg-gray-50"><Search className="w-5 h-5 text-gray-400 mr-2" /><input type="text" placeholder="Search database..." className="bg-transparent outline-none flex-1 font-medium" value={search} onChange={e => setSearch(e.target.value)} /></div><div className="max-h-[500px] overflow-y-auto"><table className="w-full text-left"><thead className="bg-gray-100 sticky top-0 shadow-sm"><tr><th className="p-4 text-xs font-bold text-gray-500 uppercase">Item Name</th><th className="p-4 text-xs font-bold text-gray-500 uppercase">Category</th><th className="p-4 text-right text-xs font-bold text-gray-500 uppercase">Action</th></tr></thead><tbody className="divide-y divide-gray-100">{filteredItems.map(item => (<tr key={item.id} className="hover:bg-blue-50 transition-colors group"><td className="p-4 font-bold text-gray-800">{item.name}</td><td className="p-4 text-sm"><span className="px-2 py-1 bg-white border rounded text-xs font-medium text-gray-500">{item.category}</span></td><td className="p-4 text-right"><button onClick={() => deleteMasterItem(item.id)} className="text-gray-300 hover:text-red-600 p-2 hover:bg-red-50 rounded-full transition-all"><Trash2 className="w-4 h-4" /></button></td></tr>))}</tbody></table></div></div>
    </div>
  );
};
