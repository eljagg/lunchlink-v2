...
interface StoreData {
  // ... existing state ...
  currentUser: User | null; currentCompany: Company | null; users: User[]; menus: DailyMenu[]; orders: Order[]; messages: Message[]; comments: Comment[]; menuIssues: MenuIssue[]; departments: Department[]; appConfig: AppConfig; masterFoodItems: MasterFoodItem[]; menuTemplates: MenuTemplate[]; isLoading: boolean;
  ...
  lockUser: (id: string, lock: boolean) => Promise<void>; addDepartment: (d: Department) => Promise<void>; updateOrderStatus: (id: string, s: Order['status']) => Promise<void>; updateAppConfig: (c: AppConfig) => Promise<void>; placeOrder: (o: Order) => Promise<void>; sendMessage: (m: Message) => Promise<void>; addComment: (c: Comment) => Promise<void>; respondToComment: (id: string, r: string, u: User) => Promise<void>; reportIssue: (i: MenuIssue) => Promise<void>; respondToIssue: (id: string, r: string) => Promise<void>; importData: (data: any) => void; exportData: () => any;
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [appConfig, setAppConfig] = useState<AppConfig>({ companyName: 'LunchLink Enterprise', tagline: 'Employee Lunch Ordering System', logoUrl: '', orderCutoffTime: '10:30' });
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        const { data: oData } = await supabase.from('orders').select('*'); if(oData) setOrders(oData.map(o => ({ id: o.id, userId: o.user_id, menuId: o.menu_id, selectedItemIds: o.selected_item_ids, date: o.date, specialInstructions: o.special_instructions, status: o.status, timestamp: o.timestamp, companyId: o.company_id })));
        ...
        const { data: tData } = await supabase.from('menu_templates').select('*'); if(tData) setMenuTemplates(tData.map(t => ({ id: t.id, name: t.name, items: t.items, notes: t.notes, createdById: t.created_by_id, createdByName: t.created_by_name, isShared: t.is_shared, createdAt: t.created_at, companyId: t.company_id })));
        ...
        if (compData) {
            setCompanies(compData.map(c => ({
                id: c.id, name: c.name, logoUrl: c.logo_url, 
                primaryColor: c.primary_color, secondaryColor: c.secondary_color, 
                welcomeMessage: c.welcome_message, tagline: c.tagline
            })));
        }
  ...
  const login = async (input: string) => {
    ...
    try {
        ...
        setCurrentUser(user);
        // We would re-fetch company-specific data here if we were using RLS strictly, but for now we rely on the initial fetch + memory filter
        setIsLoading(false);
        return true;
    } catch (e) { console.error("Login Error", e); setIsLoading(false); return false; }
  };
  ...
  const addCompany = async (c: Company) => {
      ...
      await supabase.from('companies').insert({ 
          id: c.id, name: c.name, logo_url: c.logoUrl, 
          primary_color: c.primaryColor, secondary_color: c.secondaryColor, 
          welcome_message: c.welcomeMessage, tagline: c.tagline 
      });
  };
  ...
  const updateCompany = async (c: Company) => {
      ...
      await supabase.from('companies').update({
          name: c.name, logo_url: c.logoUrl, 
          primary_color: c.primaryColor, secondary_color: c.secondaryColor, 
          welcome_message: c.welcomeMessage, tagline: c.tagline
      }).eq('id', c.id);
  };
  ...
  return (
    <StoreContext.Provider value={{
      ...
      addCompany, updateCompany, deleteCompany, // NEW
      updateAppConfig, importData, exportData
    }}>
      {children}
